...
fetch('visual-mnemonics-6.2.1.json?v=6.8.0',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json();}).then(d=>{if(!Array.isArray(d.items))throw new Error('Formato inválido');data=d;render();document.documentElement.dataset.visualMnemonicsRuntime=VERSION;window.BishunVisualMnemonics={version:VERSION,sourceVersion:SOURCE_VERSION,count:data.items.length};document.dispatchEvent(new CustomEvent('bishun:visual-mnemonics-ready',{detail:{count:data.items.length,sourceVersion:SOURCE_VERSION}}));}).catch(()=>{const grid=$('#visualMnemonicGrid');if(grid)grid.innerHTML='<p class="full-notice">Não foi possível carregar as ilustrações. Recarregue a página.</p>';});
})();
